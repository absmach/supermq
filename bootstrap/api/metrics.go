...
func (mm *metricsMiddleware) Bootstrap(externalKey, externalID string, encrypted bool) (cfg bootstrap.Config, err error) {
	defer func(begin time.Time) {
		mm.counter.With("method", "bootstrap").Add(1)
		mm.latency.With("method", "bootstrap").Observe(time.Since(begin).Seconds())
	}(time.Now())

	return mm.svc.Bootstrap(externalKey, externalID, encrypted)
}
