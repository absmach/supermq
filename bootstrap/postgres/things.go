...
func (tr thingRepository) Save(thing bootstrap.Thing) (string, error) {
	q := `INSERT INTO things (mainflux_key, owner, mainflux_thing, external_id, mainflux_channels, external_config, status) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`
	if thing.ExternalConfig == "" {
		thing.ExternalConfig = "{}"
	}

	if err := tr.db.QueryRow(q, nullString(thing.MFKey), thing.Owner, nullString(thing.MFThing),
		thing.ExternalID, pq.Array(thing.MFChannels), thing.ExternalConfig, thing.Status).Scan(&thing.ID); err != nil {
		return "", err
	}
...
func (tr thingRepository) RetrieveByID(key, id string) (bootstrap.Thing, error) {
	...
	err := tr.db.
		QueryRow(q, id, key).
		Scan(&mfKey, &mfThing, &thing.ExternalID, thing.MFChannels, &thing.ExternalConfig, &thing.Status)
...
func (tr thingRepository) RetrieveAll(key string, offset, limit uint64) []bootstrap.Thing {
	...
	for rows.Next() {
		...
		if err = rows.Scan(&mfKey, &mfThing, &t.ExternalID, t.MFChannels, &t.Status); err != nil {
...
func (tr thingRepository) RetrieveByExternalID(externalID string) (bootstrap.Thing, error) {
	...
	if err := tr.db.QueryRow(q, externalID).Scan(&thing.ID, &thing.Owner, &mfKey, &mfThing, thing.MFChannels, &thing.Status); err != nil {
		empty := bootstrap.Thing{}
		if err == sql.ErrNoRows {
			return empty, bootstrap.ErrNotFound
		}
...
func (tr thingRepository) Update(thing bootstrap.Thing) error {
	...
	res, err := tr.db.Exec(q, nullString(thing.MFKey), nullString(thing.MFThing), thing.ExternalID, thing.MFChannels, thing.ExternalConfig, thing.Status, thing.ID, thing.Owner)
	if err != nil {
		return err
	}
