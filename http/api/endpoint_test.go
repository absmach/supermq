...
func TestPublish(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.desc, func(t *testing.T) {
			clientsCall := clients.On("Authenticate", mock.Anything, &grpcClientsV1.AuthnReq{ClientSecret: smqauthn.AuthPack(smqauthn.DomainAuth, tc.key, tc.domainID)}).Return(tc.authnRes, tc.authnErr)
