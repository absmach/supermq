...
func (svc usersService) Register(ctx context.Context, token string, user User) (string, error) {
	...
	if err := svc.claimMembership(ctx, user.ID, usersObjKey, memberRelationKey); err != nil {
		return "", err
	}
...
func (svc usersService) claimMembership(ctx context.Context, subject, object, relation string) error {
