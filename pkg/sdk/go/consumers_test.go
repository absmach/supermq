...
func TestCreateSubscription(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			token:        token,
			err:          errors.NewSDKErrorWithStatus(errors.Wrap(apiutil.ErrInvalidTopic, apiutil.ErrValidation), http.StatusBadRequest),
			empty:        true,
		},
	}
...
func TestViewSubscription(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			subID:    "43",
			token:    exampleUser1,
			err:      errors.NewSDKErrorWithStatus(errors.Wrap(errors.ErrNotFound, apiutil.ErrValidation), http.StatusNotFound),
			response: sdk.Subscription{},
		},
		{
