...
func TestCreateThing(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			thing:    th1,
			token:    wrongValue,
			err:      errors.NewSDKErrorWithStatus(errors.ErrAuthentication.Error(), http.StatusUnauthorized),
			location: "",
		},
	}
...
func TestCreateThings(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			things: []sdk.Thing{},
			token:  token,
			err:    errors.NewSDKErrorWithStatus(apiutil.ErrEmptyList.Error(), http.StatusBadRequest),
			res:    []sdk.Thing{},
		},
		{
			desc:   "create new thing with empty token",
		...
		},
		{
			...
			token:  token,
			err:    errors.NewSDKErrorWithStatus(apiutil.ErrInvalidIDFormat.Error(), http.StatusBadRequest),
			res:    []sdk.Thing{},
		},
	}
	for _, tc := range cases {
...
func TestThing(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			thID:     "43",
			token:    token,
			err:      errors.NewSDKErrorWithStatus(errors.ErrNotFound.Error(), http.StatusNotFound),
			response: sdk.Thing{},
		},
		{
...
func TestThings(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			offset:   0,
			limit:    0,
			err:      errors.NewSDKErrorWithStatus(apiutil.ErrLimitSize.Error(), http.StatusBadRequest),
			response: nil,
			metadata: make(map[string]interface{}),
		},
		{
...
func TestUpdateThing(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			token: token,
			err:   errors.NewSDKErrorWithStatus(errors.ErrAuthorization.Error(), http.StatusForbidden),
		},
		{
			...
			token: token,
			err:   errors.NewSDKErrorWithStatus(apiutil.ErrMissingID.Error(), http.StatusBadRequest),
		},
		{
...
func TestIdentifyThing(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			thingKey: "",
			err:      errors.NewSDKErrorWithStatus(apiutil.ErrBearerKey.Error(), http.StatusUnauthorized),
			response: "",
		},
	}
