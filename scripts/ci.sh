...
run_test() {
    echo "Running lint..."
    golangci-lint run --no-config --disable-all --enable gosimple --enable goimports --enable gci --enable errcheck --enable govet --enable gofumpt --enable unused --enable goconst --enable godot --timeout 3m
    echo "Running tests..."
    echo "" > coverage.txt
