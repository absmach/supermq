...
func (lm *loggingMiddleware) AddPolicy(ctx context.Context, token string, clientType policies.ClientType, p policies.Policy) (policy policies.Policy, err error) {
	defer func(begin time.Time) {
	...
	return lm.svc.AddPolicy(ctx, token, clientType, p)
}
