...
func TestChannelUpdate(t *testing.T) {
	...
	nonexistentChanID, err := uuidProvider.ID()
	require.Nil(t, err, fmt.Sprintf("got unexpected error: %s", err))
...
func TestSingleChannelRetrieval(t *testing.T) {
	...
	thid, err := uuidProvider.ID()
	require.Nil(t, err, fmt.Sprintf("got unexpected error: %s", err))
	thkey, err := uuidProvider.ID()
	require.Nil(t, err, fmt.Sprintf("got unexpected error: %s", err))
	th := things.Thing{
		ID:    thid,
		Owner: email,
...
func TestRetrieveByThing(t *testing.T) {
	...
	nonexistentThingID, err := uuidProvider.ID()
	require.Nil(t, err, fmt.Sprintf("got unexpected error: %s", err))
...
func TestHasThingByID(t *testing.T) {
	...
	disconnectedThID, err := uuidProvider.ID()
	require.Nil(t, err, fmt.Sprintf("got unexpected error: %s", err))
	disconnectedThKey, err := uuidProvider.ID()
	require.Nil(t, err, fmt.Sprintf("got unexpected error: %s", err))
