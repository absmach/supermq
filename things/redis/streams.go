...
func (es eventStore) ProvisionThings(ctx context.Context, token string, ths []things.Thing) ([]things.Thing, error) {
	sths, err := es.svc.ProvisionThings(ctx, token, ths)
	if err != nil {
		return sths, err
	}
...
func (es eventStore) ProvisionChannels(ctx context.Context, token string, channels []things.Channel) ([]things.Channel, error) {
	schs, err := es.svc.ProvisionChannels(ctx, token, channels)
	if err != nil {
		return schs, err
	}
