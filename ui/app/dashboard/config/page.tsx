...
const ConfigPage = () => {
  ...
  const grouped = configItems.reduce((acc: Record<string, typeof configItems>, item) => {
  ...
  return (
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-flux-text mb-1">Configuration</h1>
          <p className="text-flux-text-muted">Broker settings from running config</p>
        </div>
        <Button
          variant="outline"
          size="sm"
      ...
      <div className="space-y-4">
        {Object.entries(grouped).map(([category, items]) => (
          <Card key={category} className="border-flux-card-border bg-flux-card overflow-hidden">
            <div className="px-6 py-3 border-b border-flux-card-border bg-flux-hover">
              <h2 className="text-sm font-semibold text-flux-text-muted uppercase tracking-wide">{category}</h2>
            </div>
            <CardContent className="p-0">
              <div className="divide-y divide-flux-card-border">
                {items.map((item, i) => (
                  <div key={i} className="px-6 py-3 flex items-center justify-between">
                    <p className="text-flux-text text-sm font-mono">{item.key}</p>
                    {isEditing ? (
                      <Input
                        type="text"
                      ...
                      <span className="text-flux-text-muted text-sm font-mono">{item.value}</span>
                    )}
                  </div>
                ))}
              </div>
