...
func TestReadAll(t *testing.T) {
	...
	for _, tc := range cases {
		...
		authCall := auth.On("Authorize", mock.Anything, mock.Anything).Return(tc.authResponse, tc.err)
		...
		if tc.key != "" {
			repoCall = tauth.On("Authorize", mock.Anything, mock.Anything).Return(tc.authResponse, tc.err)
		}
		req := testRequest{
			client: ts.Client(),
