...
func (mm *metricsMiddleware) ReadAll(chanID string, offset, limit uint64, andQuery map[string]string) []mainflux.Message {
	defer func(begin time.Time) {
		mm.counter.With("method", "read_all").Add(1)
		mm.latency.With("method", "read_all").Observe(time.Since(begin).Seconds())
	}(time.Now())

	return mm.svc.ReadAll(chanID, offset, limit, andQuery)
}
