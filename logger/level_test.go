...
func TestLevelSelectionGoodInput(t *testing.T) {
...
func TestLevelSelectionBadInput(t *testing.T) {
	cases := map[string]struct {
		input  string
		output Level
	}{
	...
	for desc, tc := range cases {
		err := logLevel.UnmarshalText(tc.input)
		assert.Error(t, err, fmt.Sprintf("unrecognized log level: %q", tc.input), desc, err)
		assert.Equal(t, tc.output, logLevel, fmt.Sprintf("%s: expected %s got %d", desc, tc.output, logLevel))
	}
}
...
func TestLevelIsNotAllowed(t *testing.T) {
	cases := map[string]struct {
		requestedLevel Level
		allowedLevel   Level
		output         bool
