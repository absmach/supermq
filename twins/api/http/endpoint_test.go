...
func TestListTwins(t *testing.T) {
	...
	twinsURL := fmt.Sprintf("%s/twins", ts.URL)
	...
	cases := []struct {
	...
	}{
		{
			...
			auth:   token,
			status: http.StatusOK,
			url:    fmt.Sprintf(queryFmt, twinsURL, 0, 5),
			res:    data[0:5],
		},
		{
			...
			auth:   wrongValue,
			status: http.StatusForbidden,
			url:    fmt.Sprintf(queryFmt, twinsURL, 0, 1),
			res:    nil,
		},
		{
		...
		},
		{
			...
			auth:   token,
			status: http.StatusOK,
			url:    fmt.Sprintf(queryFmt, twinsURL, 25, 40),
			res:    data[25:65],
		},
		{
			...
			auth:   token,
			status: http.StatusOK,
			url:    fmt.Sprintf(queryFmt, twinsURL, 91, 20),
			res:    data[91:],
		},
		{
			...
			auth:   token,
			status: http.StatusBadRequest,
			url:    fmt.Sprintf(queryFmt, twinsURL, -1, 5),
			res:    nil,
		},
		{
		...
		},
		{
			...
			auth:   token,
			status: http.StatusBadRequest,
			url:    fmt.Sprintf(queryFmt, twinsURL, 1, 0),
			res:    nil,
		},
		{
			...
			auth:   token,
			status: http.StatusBadRequest,
			url:    fmt.Sprintf("%s?offset=%d&limit=%d", twinsURL, 0, 110),
			res:    nil,
		},
		{
			...
			auth:   token,
			status: http.StatusBadRequest,
			url:    fmt.Sprintf("%s%s", twinsURL, "?offset=e&limit=5"),
			res:    nil,
		},
		{
			...
			auth:   token,
			status: http.StatusBadRequest,
			url:    fmt.Sprintf("%s%s", twinsURL, "?offset=5&limit=e"),
			res:    nil,
		},
		{
			...
			auth:   token,
			status: http.StatusOK,
			url:    fmt.Sprintf("%s?limit=%d", twinsURL, 5),
			res:    data[0:5],
		},
		{
			...
			auth:   token,
			status: http.StatusOK,
			url:    fmt.Sprintf("%s?offset=%d", twinsURL, 1),
			res:    data[1:11],
		},
		{
			...
			auth:   token,
			status: http.StatusBadRequest,
			url:    fmt.Sprintf("%s%s", twinsURL, "?offset=4&limit=4&limit=5&offset=5"),
			res:    nil,
		},
		{
			...
			auth:   token,
			status: http.StatusOK,
			url:    fmt.Sprintf("%s?offset=%d&limit=%d&value=something", twinsURL, 0, 5),
			res:    data[0:5],
		},
		{
			desc:   "get a list of twins with default URL",
			auth:   token,
			status: http.StatusOK,
			url:    fmt.Sprintf("%s%s", twinsURL, ""),
			res:    data[0:10],
		},
		{
			...
			auth:   token,
			status: http.StatusBadRequest,
			url:    fmt.Sprintf("%s?offset=%d&limit=%d&name=%s", twinsURL, 0, 5, invalidName),
			res:    nil,
		},
		{
			desc:   "get a list of things filtering with valid name",
			auth:   token,
			status: http.StatusOK,
			url:    fmt.Sprintf("%s?offset=%d&limit=%d&name=%s", twinsURL, 2, 1, twinName+"-2"),
			res:    data[2:3],
		},
	}
